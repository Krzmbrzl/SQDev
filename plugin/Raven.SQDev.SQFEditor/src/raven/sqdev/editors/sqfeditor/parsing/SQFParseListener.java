...
public class SQFParseListener extends SQFBaseListener {
				...
				if (currentChild instanceof TerminalNodeImpl && currentChild.getText().equals(";")) {
					...
					if (currentChild instanceof StatementContext) {
						...
						editor.createMarker(IMarker.PROBLEM, openStatement.getStop().getStopIndex(), 1,
								"Missing ';' at \"" + openStatement.getStop().getText() + "\"", IMarker.SEVERITY_ERROR);
	...
	/*
	 * @Override public void exitPreprocessor(PreprocessorContext ctx) { //
	 * StringBuilder input = new StringBuilder(); // // for (int i = 0; i <
	 * ctx.getChildCount(); i++) { // ParseTree current = ctx.getChild(i); // //
	 * input.append(current.getText()); // // List<Token> hiddenTokens = null;
	 * // // if (current instanceof TerminalNodeImpl) { // hiddenTokens =
	 * stream.getHiddenTokensToRight( // ((TerminalNodeImpl)
	 * current).getSymbol().getTokenIndex()); // } else { // if (current
	 * instanceof ParserRuleContext) { // hiddenTokens =
	 * stream.getHiddenTokensToRight( // ((ParserRuleContext)
	 * current).getStop().getTokenIndex()); // } // } // // if (hiddenTokens !=
	 * null) { // for (Token currentToken : hiddenTokens) { //
	 ...
	 * parser.removeErrorListeners(); // parser.addErrorListener( // new
	 * PreprocessorErrorListener(editor, // ctx.getStart().getStartIndex())); //
	 * // ParseTreeWalker walker = new ParseTreeWalker(); // // walker.walk(new
	 * PreprocessorParseListener(editor), parser.start()); }
	 */
}
