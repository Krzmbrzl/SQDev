...
public class SQFSyntaxProcessor {
	...
	/**
	 * Indicates whether the command has been validated with the provided
	 * argument types
	 */
	...
	protected void validate() {
		...
		if (!(canBeBinary || canBeUnary || canBeNular)) {
			...
			if (leftArgumentTypes != null && rightArgumentTypes != null) {
				...
				errorMessage = ProblemMessages
						.operatorIsNotBinary(command.getKeyword());
			...
			if (rightArgumentTypes != null) {
				if (command.isBinaryOperator()) {
					...
					errorMessage = ProblemMessages
							.missingArgLeft(command.getKeyword());
					...
					errorMessage = ProblemMessages
							.operatorIsNular(command.getKeyword());
			...
			errorMessage = ProblemMessages
					.operatorIsNotNular(command.getKeyword());
		...
		List<Syntax> possibleSyntaxes = new ArrayList<Syntax>(
				command.getSyntaxes());
		...
		for (int i = 0; i < possibleSyntaxes.size(); i++) {
			...
			if (currentSyntax.isBinary()) {
				...
				DataTypeList validLeftArgTypes = getTypesForSide(currentSyntax,
						true);
				...
				for (EDataType currentType : leftArgumentTypes) {
					if (validLeftArgTypes.containsExchangableType(currentType,
							true)) {
						matched = true;
		...
		if (leftArgumentTypes != null && validProvidedLeftTypes.isEmpty()) {
			...
			errorMessage = ProblemMessages.ExpectedTypeButGot(
					validGeneralLeftTypes.toArray(
							new EDataType[validGeneralLeftTypes.size()]),
					leftArgumentTypes);
		...
		for (Syntax currentSyntax : possibleSyntaxes) {
			...
			DataTypeList validRightTypes = getTypesForSide(currentSyntax,
					false);
			...
			for (EDataType currentType : rightArgumentTypes) {
				if (validRightTypes.containsExchangableType(currentType,
						true)) {
		...
		// code
		errorMessage = ProblemMessages.ExpectedTypeButGot(
				validGeneralRightTypes
						.toArray(new EDataType[validGeneralRightTypes.size()]),
				rightArgumentTypes);
	...
	/**
	 * Gets the error message stating why the current configuration of command
	 * and argument types is invalid
	 * 
	...
	/**
	 * Gets the relative position the error marker should be drawn at. The
	 * command itself is used as the center point
	 */
	 ...
	 */
	/*
	 * public boolean applyRightArgument(EDataType[] argumentTypes, int[]
	 * offsets) { return applyArgument(argumentTypes, RIGHT, offsets); }
	 */
	 ...
	 */
	/*
	 * public boolean applyLeftArgument(EDataType[] argumentTypes, int[]
	 * offsets) { return applyArgument(argumentTypes, LEFT, offsets); }
	 */
	 ...
	 * int[] offsets) { List<EDataType> possibleTypes = new
	 * ArrayList<EDataType>();
	 * 
	 ...
	 * (currentlyPossibleTypes.contains(currentType)) { return true; } else {
	 * for (EDataType validType : currentlyPossibleTypes) { if
	 * (validType.canBeReplacedBy(currentType)) { return true; } else { if
	 * (currentType.canBeReplacedBy(validType)) { // TODO: potential error
	 * return true; } } } } }
	 * 
	 ...
	 * offsets[1], ProblemMessages.ExpectedTypeButGot( possibleTypes
	 * .toArray(new EDataType[possibleTypes.size()]), argumentTypes),
	...
	private DataTypeList getTypesForSide(Syntax syntax, boolean leftSide) {
		SyntaxElement element = syntax.getElement((leftSide) ? 0
				: ((syntax.isBinary()) ? 2 : (syntax.isNular()) ? 0 : 1));
		
		if (!element.isLeafElement()) {
			throw new SQDevCoreException(
					"Expected element from the syntax for command \""
							+ syntax.getCommandName()
							+ "\" to be a leaf element");
		}
		
		DataTypeList dataTypes = new DataTypeList();
		
		for (String currentType : element.toString()
				.split(DataTypeList.TYPE_SEPERATOR)) {
			...
			if (type == null) {
				try {
					throw new SQDevException(
							"Unable to resolve type \"" + currentType + "\"");
	...
	/**
	 * Gets the return values according to the command and the syntax it is used
	 * in. If the used syntax is invalid this function will return all of the
