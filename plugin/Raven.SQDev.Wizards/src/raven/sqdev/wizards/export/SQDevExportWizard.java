...
public class SQDevExportWizard extends Wizard implements IExportWizard {
		...
		if (sel instanceof IProject) {
			...
			if (ProjectUtil.isSQDevProject(project)) {
				...
				SQDevInfobox info = new SQDevInfobox(
						"This type of export wizard is designated for the export of SQDevProjects !",
						SWT.ICON_ERROR);
				info.open();
			}
		} else {
	...
	public boolean performFinish() {
		...
		try {
			Path exportPath = (Path) page.getDestination().append(project.getName() + "."
					+ linkFile.parseAttribute(ESQDevFileAttribute.TERRAIN).getValue());
					
			ArrayList<String> ignore = linkFile.parseAnnotation(ESQDevFileAnnotation.IGNORE)
					.getValues();
					
			ArrayList<String> preserve = linkFile.parseAnnotation(ESQDevFileAnnotation.PRESERVE)
					.getValues();
