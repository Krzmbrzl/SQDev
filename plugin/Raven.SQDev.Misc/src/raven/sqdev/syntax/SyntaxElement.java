...
public class SyntaxElement {
	...
	/**
	 * Creates a <code>SyntaxElement</code> as a leaf element representing the given
	 * String
	 * 
	 * @param leafElement
	 *            The String that is represented by this <code>SyntaxElement</code>.
	 *            It may only be one word and it has to be recognized by
	 *            {@link ESQFDataType}!
	 * @throws BadSyntaxException
	 *             If the leafElement could not be resolved to a data type
	 */
	public SyntaxElement(String leafElement) throws BadSyntaxException {
	 ...
	 *            The String that is represented by this <code>SyntaxElement</code>.
	 *            It may only be one word and it has to be recognized by
	 *            {@link ESQFDataType} unless it is the command's name!
	 * @param isCommandName
	 *            Indicates whether the given element is the command's name
	 * @throws BadSyntaxException
	 *             If the leafElement could not be resolved to a data type
	 */
	public SyntaxElement(String leafElement, boolean isCommandName) throws BadSyntaxException {
		Assert.isTrue(leafElement != null && !leafElement.isEmpty());

		if (!isCommandName && !isValidDataType(leafElement)) {
			throw new BadSyntaxException("Unknown data type: " + leafElement);
		}
	...
	/**
	 * Creates a <code>SyntaxElement</code> as a node element representing the given
	 * <code>Syntax</code> as a subSyntax
	 * 
	 * @param subSyntax
	...
	/**
	 * Checks whether the given data type is valid for this syntax element
	 * 
	 * @param type
	 *            The type to check
	 */
	public boolean isValidDataType(String type) {
		return ESQFDataType.isValidDataTypeSet(type);
	}
	...
	public static SyntaxElement parseSyntaxElement(String input) throws BadSyntaxException {
		return parseSyntaxElement(input, false);
	}
	 ...
	 *            The input to parse the element from
	 * @param isCommandName
	 *            Indicates whether the given element represents the commandName
	 *            itself. That means that it does not have to be a dataType
	 * @return The parsed <code>SyntaxElement</code>
	 * @throws BadSyntaxException
	 *             If the input is onvalid
	 */
	public static SyntaxElement parseSyntaxElement(String input, boolean isCommandName) throws BadSyntaxException {
		if (input == null || (input = input.trim()).isEmpty()) {
			throw new BadSyntaxException("The given input cannot be parsed into a syntaxElement (Empty or null)!");
		}
		...
		if (elements.length > 1) {
			...
			if (encapsulator == null) {
				...
				return ((isCommandName) ? new SyntaxElement(elements[0], true) : new SyntaxElement(elements[0]));
			} else {
				Syntax subSyntax = new Syntax();
