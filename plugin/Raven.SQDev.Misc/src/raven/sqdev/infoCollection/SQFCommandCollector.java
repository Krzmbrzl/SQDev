...
public class SQFCommandCollector {
	...
	/**
	 * A set of essential commands that are forced to be included in the collection
	 * process
	 */
	public static final String[] ESSENTIAL_COMMANDS = { "if", "then", "else", "while", "do", "for", "with", "switch",
			"from", "to", "step", "count", "forEach" };

	/**
	 * A list of operators that are not retrieved by the normal command collection
	 * as they are listed in a different format
	 */
	 ...
	 */
	public static final String[] MANUAL_COMMANDS_DESCRIPTION = { "Negates the given Boolean value. Synonym for 'not'.",
	 ...
	 */
	public static final String[] MANUAL_COMMANDS_WIKI = { "https://community.bistudio.com/wiki/!_a",
			"https://community.bistudio.com/wiki/a_%3D%3D_b", "https://community.bistudio.com/wiki/a_!%3D_b",
			"https://community.bistudio.com/wiki/a_less_b", "https://community.bistudio.com/wiki/a_less%3D_b",
			"https://community.bistudio.com/wiki/a_greater_b", "https://community.bistudio.com/wiki/a_greater%3D_b",
			"https://community.bistudio.com/wiki/a_%26%26_b", "https://community.bistudio.com/wiki/a_or_b",
			"https://community.bistudio.com/wiki/valuea_plus_valueb", "https://community.bistudio.com/wiki/a_-_b",
			"https://community.bistudio.com/wiki/a_*_b", "https://community.bistudio.com/wiki/a_/_b",
			"https://community.bistudio.com/wiki/a_%5E_b", "https://community.bistudio.com/wiki/a_%25_b",
	...
	public KeywordList collect(IProgressMonitor monitor, URL repeat) throws SQDevCollectionException {
		...
		try {
			...
			// rethrow exception
			throw new SQDevCollectionException(e1.getMessage(), null, new KeywordList());
		}
		...
		for (int i = 0; i < MANUAL_COMMANDS.length; i++) {
			...
			SQFCommand command = new SQFCommand(currentCommand, currentDescription);

			for (int k = 0; k < MANUAL_COMMANDS_SYNTAX[i].length; k++) {
				String currentSyntax = MANUAL_COMMANDS_SYNTAX[i][k];
				DataTypeList returnTypes = DataTypeList.fillWith(MANUAL_COMMANDS_RETURN_TYPE[i][k]);
				try {
					Syntax syntax = Syntax.parseSyntax(currentSyntax, currentCommand);
		...
		for (int i = 0; i < size; i++) {
			...
			// display which command currently is processed
			monitor.subTask("Current Command: \"" + name + "\"");

			SQFCommand command = processCommand(new SQFCommand(name), currentCommandURL);
	...
	/**
	 * Gets the content of the specified site
	 * 
	 * @param url
	 *            The URL to the site
	 * @param removeHTMLTags
	 *            Whether all HTML tags should get removed
	...
	public static String getSite(URL url, boolean removeHTMLTags) throws IOException {
		...
		while (matcher.find()) {
			...
			char escapeCharacter = (char) Integer.parseInt(fragment.substring(2, fragment.length() - 1));
	...
	/**
	 * Will process the given commandPage and feed the gathered information into the
	 * given <code>SQFCommand</code>
	 * 
	 * @param command
	...
	private SQFCommand processCommand(SQFCommand command, URL commandPage) throws SQDevCollectionException {
		...
		String siteContent;
		try {
			siteContent = formatCommandPageContent(getSite(commandPage, false));
		} catch (IOException e) {
			// rethrow
		...
		if (!commandPage.toString().endsWith(command.getKeyword())
				|| !categories[CATEGORY_SYNTAX].toLowerCase().contains(command.getKeyword().toLowerCase())) {
		...
		try {
			...
			if (!syntax.isEmpty()) {
				if (command.getKeyword().toLowerCase().equals("private")) {
					...
					syntax = syntax.substring(0, syntax.toLowerCase().lastIndexOf("alternative syntax"));
				}

				applySyntax(command, syntax);
			} else {
				throw new SQDevCollectionException(
						"The command \"" + command.getKeyword() + "\" does not specify a syntax!", commandPage, list);
			}
		...
		for (String currentLine : content.split("\n")) {
			...
			if (currentCategory.isEmpty() && currentLine.startsWith("Posted")) {
				...
				if (!currentCategory.isEmpty()
						&& (currentLine.startsWith("Categories") || currentLine.startsWith("Retrieved from"))) {
					currentCategory = "";
				}
			}
	 ...
	 */
	private void applyCommandInfo(SQFCommand command, String info) throws SQDevException {
		String name = info.substring(info.indexOf("Name:") + 5, info.indexOf("\n")).trim();

		if (!name.toLowerCase().equals(command.getKeyword().toLowerCase())) {
			throw new SQDevException("The given commandInfo does not specify the same name (\"" + name
	 ...
	 * @param examples
	 *            The String containing the examples indexed by "Example 1:" and so
	 *            on
	 */
	private void applyExamples(SQFCommand command, String examples) {
		...
		while (proceed) {
			...
			if (examples.startsWith(exampleNum)) {
				...
				examples = examples.substring(examples.indexOf(exampleNum) + exampleNum.length()).trim();
				...
				if (examples.contains(nextExampleNum)) {
					...
					String currentExample = examples.substring(0, examples.indexOf(nextExampleNum)).trim();
	 ...
	 * @throws SQDevException
	 */
	private void applyNotes(SQFCommand command, String notes) throws SQDevException {
		String currentNote = "";
		boolean skippedName = false;

		if (!notes.contains("Posted on")) {
			throw new SQDevException("The notes of " + command.getKeyword() + "are not in the proper format!");
		}
	...
	/**
	 * Applies the given syntax to the given command after having brought it into
	 * the proper format
	 * 
	 * @param command
	 *            The command the notes should be applied to
	 ...
	 * @throws SQDevException
	 */
	private void applySyntax(SQFCommand command, String syntaxContent) throws SQDevException {
		...
		for (String[] currentSyntax : syntaxes) {
			...
			DataTypeList returnTypes = DataTypeList.fillWith(currentSyntax[SYNTAXPART_RETURN_VALUE]);
			...
			try {
				if (syntax.contains(String.valueOf(OPTIONAL_MARKER))) {
					...
					for (int i = 0; i < areas.length; i++) {
						...
						if (currentArea.toLowerCase().equals(command.getKeyword().toLowerCase())) {
							commandIndex = i;
							continue;
						}

						if (currentArea.endsWith(String.valueOf(OPTIONAL_MARKER))) {
							if (commandIndex < 0) {
								...
								for (int j = 0; j < i; j++) {
									...
									builder.append(areas[j].replace(String.valueOf(OPTIONAL_MARKER), "") + " ");
								}

								possibleLeadingArgCombinations.add(builder.toString().trim());
							} else {
								...
								possibleTrailingArgCombinations.add(builder.toString().trim());
							}
						}
					}
					...
					for (String currentLeadingCombination : possibleLeadingArgCombinations) {
						for (String currentTrailingCombination : possibleTrailingArgCombinations) {
							Syntax parsedSyntax = Syntax.parseSyntax(currentLeadingCombination + " "
									+ command.getKeyword() + " " + currentTrailingCombination, command.getKeyword());
					...
					if (possibleLeadingArgCombinations.size() + possibleTrailingArgCombinations.size() != 0) {
						// add complete syntax
						Syntax parsedSyntax = Syntax.parseSyntax(syntax.replace(String.valueOf(OPTIONAL_MARKER), ""),
								command.getKeyword());
	 ...
	 * @throws SQDevException
	 */
	private String processSyntax(String syntax, String parameter, SQFElement element) throws SQDevException {
		for (String currentParameter : parameter.split("param:\n")) {
			...
			if (!elements[0].contains(":") || elements.length == 1) {
				if (elements.length > 2 && (elements[1].equals(":") || elements[1].equals("-"))) {
					// add needed colon
					elements[0] = elements[0] + ":";
					// Move parameter data type
					elements[1] = elements[2];
				} else {
					throw new SQDevException("Unexpected parameter format - missing\":\" at " + elements[0]);
				}
			}

			String parameterName = elements[0].substring(0, elements[0].indexOf(":")).toLowerCase();
	...
	private String formatSyntaxContent(String syntaxContent) {
		...
		syntaxContent = syntaxContent.substring(0, syntaxContent.indexOf("Return Value:") + 13).trim() + " "
		...
		syntaxLine = syntaxLine.replaceAll("[sS]ince [aA]rm[aA][^)]*", "").replaceAll("[(][^)]*\\z", "");

		return syntaxLine + syntaxContent.substring(syntaxContent.indexOf("\n"));
	}

	/**
	 * Splits the given syntax(es) and return them in an array (everything properly
	 * formatted)
	 * 
	 * @param syntaxContent
	 ...
	 *         different syntaxes (alternatives syntaxes) and the second dimension
	 *         is as following:
	 ...
	 * @throws SQDevException
	 */
	private String[][] splitSyntaxes(String syntaxContent, SQFElement element) throws SQDevException {
		...
		for (String currentSyntaxContent : syntaxContent.split("Alternative Syntax")) {
			...
			// get the parameters
			String currentParameter = currentSyntaxContent.substring(0, currentSyntaxContent.indexOf("Return Value:"));
			...
			currentSyntaxComponents.add(SYNTAXPART_PARAMETERS, formatParameters(currentParameter, element));
			currentSyntaxComponents.add(SYNTAXPART_RETURN_VALUE,
					formatReturnValue(currentReturnValue, element.getKeyword()));
	 ...
	 * @throws SQDevException
	 */
	private String formatParameters(String parameters, SQFElement element) throws SQDevException {
		...
		for (int i = 0; i < areas.length; i++) {
			...
			if (currentArea.toLowerCase().contains("optional")) {
				if (!markedOptional) {
					...
					if (dataTypeEndOffset < 0 || builder.length() - dataTypeEndOffset < 40
							|| ArrayUtils.containsElementInmaxDistance(areas, "$param$", i, 3)) {
			...
			if (currentArea.startsWith("(")) {
				...
				default:
					if (matchedColon) {
						...
						if (optionalMarkingNeeded && Character.isLetter(currentArea.trim().charAt(0))) {
				...
				if (matchedDataType && dataTypeEndOffset < 0 && Character.isLetter(currentArea.trim().charAt(0))) {
					dataTypeEndOffset = builder.length() + currentArea.length() + 1;
				}
		...
		parameters = builder.toString().trim().replace(" :", ":").replace(" / ", DataTypeList.TYPE_SEPERATOR);
	...
	private String formatAlternativeDataTypes(String input) {
		...
		relevantPart = relevantPart.replace(",", DataTypeList.TYPE_SEPERATOR).replaceAll("\\bor\\b",
	 ...
	 * @throws SQDevException
	 */
	private String formatReturnValue(String returnValue, String commandName) throws SQDevException {
		...
		returnValue = returnValue.replace(",", DataTypeList.TYPE_SEPERATOR).replaceAll("\\bor\\b",
				DataTypeList.TYPE_SEPERATOR);

		// remove leading whitespace in fromt of seperator
		while (returnValue.contains(" " + DataTypeList.TYPE_SEPERATOR)) {
			returnValue = returnValue.replace(" " + DataTypeList.TYPE_SEPERATOR, DataTypeList.TYPE_SEPERATOR);
		}
	 ...
	 */
	private String formatDataType(String type, String commandName) throws SQDevException {
		StringBuilder newDataType = new StringBuilder();

		boolean isOptionalType = type.trim().endsWith(String.valueOf(OPTIONAL_MARKER));
		...
		for (String currentType : type.split(DataTypeList.TYPE_SEPERATOR)) {
			...
			if (dataType == null) {
				...
				newDataType.append((newDataType.length() == 0) ? dataType.toString()
						: DataTypeList.TYPE_SEPERATOR + dataType.toString());
			}
		}

		if (newDataType.length() == 0) {
			throw new SQDevException("Empty data type in command \"" + commandName + "\"");
		}
