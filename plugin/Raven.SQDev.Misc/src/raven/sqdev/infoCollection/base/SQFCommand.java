...
public class SQFCommand extends SQFElement {
	 ...
	 * relocated to {@link ESQFDataType} as it makes more sense to specify it there.
	 * This field does only support for backwards-compability reasons.
	 * 
	 * @deprecated
	 */
	public static final String TYPE_SEPERATOR = DataTypeList.TYPE_SEPERATOR;
	/**
	 * The seperator used in order to show that the different sets of return values
	 * correspond to a different syntax
	 */
	...
	/**
	 * The sequence seperating multiple syntaxes in the saveable String format of
	 * this class
	 */
	public static final String SYNTAX_SEPERATOR_SAVESEQUENCE = "</NextSyntax>";
	/**
	 * The sequence indicating the start of the rawSyntax attribute in the saveable
	 * String format of this class
	 */
	public static final String RAWSYNTAX_START_SAVESEQUENCE = "<RawSyntax>";
	/**
	 * The sequence indicating the end of the rawSyntax attribute in the saveable
	 * String format of this class
	 */
	public static final String RAWSYNTAX_END_SAVESEQUENCE = "</RawSyntax>";
	/**
	 * The sequence seperating multiple rawSyntaxes in the saveable String format of
	 * this class
	 */
	public static final String RAWSYNTAX_SEPERATOR_SAVESEQUENCE = "</NextRawSyntax>";
	/**
	 * The sequence indicating the start of the example attribute in the saveable
	 * String format of this class
	 */
	...
	/**
	 * The sequence seperating multiple examples in the saveable String format of
	 * this class
	 */
	public static final String EXAMPLE_SEPERATOR_SAVESEQUENCE = "</NextExample>";
	/**
	 * The sequence indicating the start of the locality attribute in the saveable
	 * String format of this class
	 */
	...
	/**
	 * The sequence seperating multiple notes in the saveable String format of this
	 * class
	 */
	...
	/**
	 * The sequence indicating the end of the returnValue attribute in the saveable
	 * String format of this class
	 */
	...
	/**
	 * The syntaxes of this command in their raw form without having been processed
	...
	/**
	 * An array containing the locality of this command's arguments (index 0) and
	 * the locality of it's effect (index 1)
	 */
	...
	/**
	 * Gets the syntaxes of this command. If the syntaxlist has not been initialized
	 * yet this will initialize it as an empty list and then returns that newly
	 * created list.
	 * 
	 ...
	 * 
	 * @param syntax
	 *            The syntax to add. If it is <code>null</code> nothing will be done
	 */
	 ...
	 * 
	 * @return A list containing the examples (empty list if there are no examples)
	 */
	...
	/**
	 * Gets the locality of this command's arguments (index 0) and the locality of
	 * it's effect (index 1)
	 * 
	 ...
	 * 
	 * @return The respective <code>DataTypeList</code> or <code>null</code> if no
	 *         return value for the given syntax could be found
	 */
	...
	public DataTypeList getAllReturnTypes() {
		synchronized (returnValues) {
			...
			Iterator<Entry<Syntax, DataTypeList>> it = returnValues.entrySet().iterator();
	 ...
	 * Different return values have to be seperated with type seperator specified in
	 * {@link ESQFDataType}
	 * 
	 * @param syntax
	...
	public void setReturnType(Syntax syntax, String types) {
		if (types == null || types.trim().isEmpty()) {
			setReturnType(syntax, new DataTypeList(ESQFDataType.NOTHING));
			return;
		}
	 ...
	 *            empty a new list will be created that only contains the NOTHING
	 *            type
	 */
	public void setReturnType(Syntax syntax, DataTypeList types) {
		...
		if (types == null || types.isEmpty()) {
			types = new DataTypeList(ESQFDataType.NOTHING);
		}
	...
	/**
	 * Gets the raw syntaxes of this command. These are for display purposes only as
	 * they have not been processed with their parameters
	 */
	 ...
	 * 
	 * @param rawSytaxes
	 *            The raw (unprocessed) syntaxes for this command that can be used
	...
	public String getSaveableFormat() {
		...
		for (Syntax currentSyntax : getSyntaxes()) {
			...
			if (types == null) {
				try {
					throw new SQDevException("Unable to retrieve return values for syntax \"" + currentSyntax + "\"");
	...
	public boolean recreateFrom(String savedFormat) throws BadSyntaxException {
		...
		for (String currentSyntax : syntaxContent.split(SYNTAX_SEPERATOR_SAVESEQUENCE)) {
			...
			if (!currentSyntax.isEmpty()) {
				addSyntax(createSyntax(currentSyntax, getKeyword()));
			}
		}
		...
		if (shouldHaveLocality()) {
			...
			// set undefined locality for O2Script-commands
			setArgumentLocality(ELocality.UNDEFINED);
			setEffectLocality(ELocality.UNDEFINED);
		}
		...
		// return type
		String returnTypes = savedFormat.substring(
		...
		String[] returnValues = returnTypes.split(RETURN_VALUE_SYNTAX_SEPERATOR);

		return doMapReturnValues(syntaxes, returnValues);
	}

	/**
	 * Maps the given return values to the stored syntaxes based on their index
	 * 
	 * @param syntaxes
	 *            The syntaxes to use
	 * @param returnValues
	 *            The return values corresponding to those syntaxes
	...
	protected boolean doMapReturnValues(List<Syntax> syntaxes, String[] returnValues) {
		if (syntaxes.size() != returnValues.length) {
			try {
				throw new SQDevException("savable format has to contain as many return values as syntaxes");
			} catch (SQDevException e) {
				// TODO: log
				e.printStackTrace();
			}
		...
		for (int i = 0; i < returnValues.length; i++) {
			setReturnType(syntaxes.get(i), DataTypeList.fillWith(returnValues[i]));
		}
	...
	public boolean isSaveFormat(String format) {
		...
		if (!format.contains(SYNTAX_START_SAVESEQUENCE) || !format.contains(SYNTAX_END_SAVESEQUENCE)
				...
				|| !format.contains(NOTE_START_SAVESEQUENCE) || !format.contains(NOTE_END_SAVESEQUENCE)
				|| !format.contains(RETURNVALUE_START_SAVESEQUENCE) || !format.contains(RETURNVALUE_END_SAVESEQUENCE)) {
		...
		int localitySeparator = format.indexOf(LOCALITY_SEPERATOR_SAVESEQUENCE, localityStart);
		...
		if (syntaxEnd < syntaxStart || rawSyntaxEnd < rawSyntaxStart || exampleEnd < exampleStart
				|| (shouldHaveLocality() && (localityEnd < localityStart || localitySeparator < localityStart
						|| localitySeparator > localityEnd))
	...
	/**
	 * Creates a {@linkplain Syntax} out of the given String
	 * 
	 * @param syntaxString
	 *            The string to create the syntax from
	 * @param keyword
	 *            The keyword the syntax is to be created for
	 * @return The created syntax or <code>null</code> if no syntax was created
	 * @throws BadSyntaxException
	 *             If the format of the syntax string is wrong or semthing is wrong
	 *             with the syntax in general
	 */
	protected Syntax createSyntax(String syntaxString, String keyword) throws BadSyntaxException {
		return Syntax.parseSyntax(syntaxString, keyword);
	}

	/**
	 * Checks whether this command should have a locality. Its use is to be able to
	 * use the functions defined inside {@linkplain SQFCommand} for
	 * {@linkplain O2ScriptCommand} as well.
	 */
