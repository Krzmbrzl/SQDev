...
public class SQFCommandPageProvider {
	...
	/**
	 * The total amount of pages originally found
	 */
	private static int totalPageAmount;
	 ...
	 */
	public SQFCommandPageProvider(String apiAdress, String mainPageName) throws IOException {
	 ...
	 * 
	 * @throws IOException
	 *             Thrown if something went wrong during the connection to the BIKI
	 */
	protected void initialize() throws IOException {
		final String base = getApiAdress() + "?action=query&list=categorymembers&cmtitle=" + getMainPageName()
				+ "&cmlimit=500&format=jsonfm&cmtype=page";
		...
		while (content.contains("cmcontinue")) {
			...
			final String cont = content.substring(matcher.start(), matcher.end()).replace("\"", "").replace(" ", "")
					.replace(":", "=").replace(",", "");
		...
		final Matcher idMatcher = Pattern.compile("\"pageid\":.*?,").matcher(siteContent);

		final Stack<String> pageIDs = new Stack<String>();

		while (idMatcher.find()) {
			// add page ID by cutting away the irrelevant part of the line
			pageIDs.add(siteContent.substring(idMatcher.start() + 10, idMatcher.end() - 1));
		}
		...
		while (!pageIDs.isEmpty()) {
			...
			urlPageContent.append(SQFCommandCollector.getSite(new URL(urlSearchAdress.toString()), true));
		}

		// strip out the actual URLs from this page
		commandURLs = new Stack<URL>();

		final Matcher urlMatcher = Pattern.compile("\"fullurl\":\\s*\".*?\",").matcher(urlPageContent.toString());
		while (urlMatcher.find()) {
			commandURLs.add(new URL(
					urlPageContent.substring(urlMatcher.start() + 10, urlMatcher.end() - 1).replace("\"", "").trim()));
		}

		totalPageAmount = commandURLs.size();
	}

	/**
	 * Gets the URL to the wiki page of the next command
	 * 
	 * @return The respective URL or <code>null</code> if there are no more (or none
	 *         could be found)
	...
	/**
	 * Gets the total amount of pages originally found. Therefore this is equal to
	 * the stack's original size before any URLs have been removed from it.
	 */
	...
	public static void main(String[] args) throws IOException {
		new SQFCommandPageProvider("https://community.bistudio.com/wikidata/api.php",
