...
public class ProjectUtil {
	 ...
	 * (prefixed by a dot) will be ignored automatically during export (not
	 * during cleaning)
	 * 
	 * @param project
	...
	public static String importAsProject(Path path) {
		...
		try {
			// create the project
			IProject project = createSQDevProject(
					ResourcesPlugin.getWorkspace().getRoot(), projectName);
		...
		} catch (CoreException | FailedAtCreatingFileException
	 ...
	 *             If there is a problem with creating the problem such as
	 *             another problem with the same name does already exist
	 * @throws CoreException
	 * @throws BackingStoreException
	 */
	public static IProject createSQDevProject(IWorkspaceRoot root,
			String projectName) throws FailedAtCreatingFileException,
			CoreException, BackingStoreException {
		...
		if (!project.exists()) {
			...
			throw new FailedAtCreatingFileException("A project with the name \""
					+ projectName + "\" does already exist!");
		}
		...
		description.setNatureIds(
				new String[] { SQDevProjectNature.SQDEV_NATURE_ID });
