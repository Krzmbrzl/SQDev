...
public class SyntaxElement {
	 ...
	 *            <code>SyntaxElement</code>. It may only be one word and it has
	 *            to be recognized by {@link EDataType}!
	 ...
	 *            The String that is represented by this
	 *            <code>SyntaxElement</code>. It may only be one word and it has
	 *            to be recognized by {@link EDataType} unless it is the
	 *            command's name!
	 * @param isCommandName
	 ...
	 *             If the leafElement could not be resolved to a data type
	 */
	public SyntaxElement(String leafElement, boolean isCommandName)
			throws BadSyntaxException {
		Assert.isTrue(leafElement != null && !leafElement.isEmpty());
		
		if (!isCommandName && EDataType.isValidDataTypeSet(leafElement)) {
	 ...
	 */
	public static SyntaxElement parseSyntaxElement(String input)
			throws BadSyntaxException {
	 ...
	 */
	public static SyntaxElement parseSyntaxElement(String input,
			boolean isCommandName) throws BadSyntaxException {
		...
		if (!Character.isLetterOrDigit(startingChar)) {
			...
			if (encapsulator != null
					&& encapsulator.equals(CharacterPair.TAG)) {
		...
		if (elements.length > 1) {
			...
			if (encapsulator == null) {
				...
				return ((isCommandName) ? new SyntaxElement(elements[0], true)
						: new SyntaxElement(elements[0]));
			} else {
				Syntax subSyntax = new Syntax();
				...
				value = value.substring(
						value.indexOf(encapsulator.getOpener()) + 1,
