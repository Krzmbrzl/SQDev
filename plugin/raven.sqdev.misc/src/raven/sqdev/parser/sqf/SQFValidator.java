...
public class SQFValidator extends SQFBaseListener {
	 ...
	 *            parse tree
	 */
	public SQFValidator(ISQFParseInformation info, SQFParseResult parseResult,
	...
	public void exitBinaryExpression(BinaryExpressionContext ctx) {
		...
		if (operator != null) {
			...
			if (!processor.isValid()) {
				...
				parseResult.addMarker(IMarker.PROBLEM, offsets[0], offsets[1], errorMsg,
						IMarker.SEVERITY_ERROR);
			}
	...
	protected int[] getStartOffsetAndLength(ParseTree node) {
		...
		if (startNode == null) {
			SQDevInfobox info = new SQDevInfobox(
					"Error while searching for the start index of \"" + node.getText() + "\"",
					SWT.ICON_ERROR);
			...
			if (endNode == null) {
				SQDevInfobox info = new SQDevInfobox(
						"Error while searching for the end index of \"" + node.getText() + "\"",
						SWT.ICON_ERROR);
		...
		case "private":
			if (argument.getChild(0) instanceof ArrayContext) {
				...
				if (argument.getChild(0) instanceof StringContext) {
					...
					getVariableDeclaration((TerminalNodeImpl) argument.getChild(0).getChild(0),
